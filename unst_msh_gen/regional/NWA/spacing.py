...
def tanh_wavelength_spacing(
        elev, hmin, hmax, hcenter=100, hbeta=0.02, 
        grav=9.80665):
    ...
    print(f"Scaling hmin by {hscl:.3f} to ensure it is reached at H = 1 m depth")
    
    vals = np.maximum(1, -elev)
    vals = (hmax-hscl*hmin)*0.5*(1 + np.tanh(hbeta*(vals-hcenter))) + hscl*hmin

    vals[np.logical_and(elev >= -4., elev <= 4.)] = hmin # set values within +- 4 m to hmin
