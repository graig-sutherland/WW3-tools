...
#-- create a simple uniform mesh for the globe
    ...
    opts.geom_file = "data/geom.msh"
    opts.hfun_file = "data/spac.msh"
    opts.jcfg_file = "data/opts.jig"
    ...
    opts.mesh_file = "data/mesh_25km_to_5km_tanh.msh"
    
    opts.hfun_scal = "absolute"
    opts.hfun_hmax = +25.           # uniform at 30.km
    opts.hfun_hmin  = +5.
...
#-- create mesh spacing function for the globe
    ...
    hmax = 20.0 # maximum spacing [km]
    hmin = 5.0  # minimum spacing
    hshr = 5    # shoreline spacing
    nwav = 0.  # number of cells per sqrt(g*H)
    dhdx = 0.01  # allowable spacing gradient

#    gebcoDir = '/home/gsu000/l5/GEBCO'
#    bathyFile = os.path.join(gebcoDir, 'GEBCO_2023_sub_ice_topo.nc')
...
#-- h(x) data equal to sqrt(elevation)
    hmat = np.sqrt(np.maximum(-elev,10)).astype(spac.FLT32_t)
#-- elevation gradient spacing
#    nslp = 1
#    sdev = 2
#    hmat = elev_sharpness_spacing(xlon, ylat, elev, dhdx, land, nslp, hmin, hmax, sdev)
...
#        12.5 * (ymat - ymid) ** 2) ** 2)
    ...
    # apply a tanh scaling in latitude 
    max_scale = hmax/hmin
    min_scale = 1.0
    mid_point = 0.5*(min_scale + max_scale)
    amp_scale = -1.0*(max_scale - mid_point)
    y0 = 10.0 * np.pi/180
    spac.value *= amp_scale*np.tanh(4*(ymat-y0)) + mid_point
    spac.value = np.maximum(hmin, spac.value)
    spac.value = np.minimum(hmax, spac.value)
...
#-- require dry cells > 1 dry edge, and large
    ...
    # delete groups of dry if too small (was commented out before)
...
if (__name__ == "__main__"):
    ...
    # viz. in eg. paraview
    jigsawpy.savevtk("data/20km_to_3km_tanh.vtk", mesh)
    ...
    write_gmsh_mesh("data/20km_to_3km_tanh.ww3", point, tri_data)
