...
#-- create a simple uniform mesh for the globe
    ...
    opts.mesh_file = "data/mesh_ARC_5km_50N.msh"
    
    opts.hfun_scal = "absolute"
    opts.hfun_hmax = +5.           # uniform at 30.km
    opts.hfun_hmin  = +5.
...
#-- use the remapped elev. to keep ocean cells
    ...
    # Define Arctic Region
    not_arctic_lat_min = -90.0
    not_arctic_lat_max = 50.0
    not_arctic_lon_min = -180
    not_arctic_lon_max = 180
    
    # zssh, to cull elev. against
    surf = -10*np.ones(elev.shape, dtype=np.float32)

    # Update the surf array to include both regions
    not_arctic_region = np.logical_and.reduce((
        mesh.smids[:, 1] >= not_arctic_lat_min,
        mesh.smids[:, 1] <= not_arctic_lat_max,
        mesh.smids[:, 0] >= not_arctic_lon_min,
        mesh.smids[:, 0] <= not_arctic_lon_max
    ))
    
#    # arctic domain by default
    surf[not_arctic_region] = -99999.0
...
if (__name__ == "__main__"):
    ...
    # viz. in eg. paraview
    jigsawpy.savevtk("data/ARC_5km_50N.vtk", mesh)
    ...
    write_gmsh_mesh("data/ARC_5km_50N.ww3", point, tri_data)
